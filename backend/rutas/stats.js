...
router.get('/', async (req, res) => {
  try {
    ...
    const egresadosCount = await db.query('SELECT COUNT(*) as total FROM egresados WHERE ceremonia_id = ?', [ceremoniaId]);
    ...
    const ingresosQuery = `
      ...
      FROM invitados i
      JOIN egresados e ON i.egresado_id = e.id
      WHERE i.presente = 1 AND e.ceremonia_id = ?
