...
export function GestionCeremonias({ onVolver, onCambioCeremonia }) {
  ...
  const [nuevoEvento, setNuevoEvento] = useState({
    nombre: '', fecha: '', lugar: 'Sede Beltrán', max_invitados: 5, max_entregadores: 1
  })
          ...
          <div className="bg-white rounded-[48px] shadow-2xl w-full max-w-xl overflow-hidden animate-in zoom-in duration-300">
            ...
            <form onSubmit={handleCrear} className="p-12 space-y-8">
              ...
              <div className="flex-1 space-y-1">
                <label className="text-xs font-bold text-slate-500 uppercase">Max. Invitados por Graduado</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-sky-500 transition-colors"
                    value={nuevoEvento.max_invitados}
                    onChange={(e) => setNuevoEvento({ ...nuevoEvento, max_invitados: parseInt(e.target.value) })}
                  />
                </div>
              </div>

              <div className="flex-1 space-y-1">
                <label className="text-xs font-bold text-slate-500 uppercase">Max. Entregadores Medalla</label>
                <div className="relative">
                  <div className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400">
                    <Users size={16} />
                  </div>
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-sky-500 transition-colors"
                    value={nuevoEvento.max_entregadores || 1}
                    onChange={(e) => setNuevoEvento({ ...nuevoEvento, max_entregadores: parseInt(e.target.value) })}
                  />
                </div>
              </div>
            </div>
