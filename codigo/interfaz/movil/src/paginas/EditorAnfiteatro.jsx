...
export function EditorAnfiteatro({ ceremoniaId, onVolver }) {
  ...
  // Autoguardado cuando se edita la estructura o los roles
  useEffect(() => {
    if (!cargando && ceremoniaId) {
      const timer = setTimeout(() => {
        handleGuardar(true);
      }, 500);
      return () => clearTimeout(timer);
    }
  }, [estructura, mapaRoles, cargando]);
  ...
  async function handleGuardar(silencioso = false) {
    ...
    try {
      ...
      if (!silencioso) setMensaje({ tipo: 'error', texto: 'Error al sincronizar con el servidor' });
    } finally {
