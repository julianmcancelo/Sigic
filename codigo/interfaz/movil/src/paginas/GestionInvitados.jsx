...
export function GestionInvitados({ usuario, onVolver, onCerrarSesion }) {
  ...
  const [asignandoAsientosA, setAsignandoAsientosA] = useState(null)
  ...
  return (
    <div className="min-h-screen" style={{ background: BG }}>
          ...
          {cargando ? (
            ...
            egresadosFiltrados.map(egr => (
              <TarjetaEgresado 
                ...
                onLink={(egr) => {
                  const url = `${window.location.origin}/carga?token=${egr.token_magico}`
                  setLinkQR({ egresado: egr, link: url })
                }}
                onAsignarAsientos={(egr) => setAsignandoAsientosA(egr)}
      ...
      {/* MODAL ASIGNAR ASIENTOS */}
      {asignandoAsientosA && (
        <ModalAsignarAsientos 
          graduado={asignandoAsientosA}
          invitados={invitadosDe(asignandoAsientosA.id)}
          onCerrar={() => setAsignandoAsientosA(null)}
          onCompletado={() => {
            setAsignandoAsientosA(null)
            cargarDatos()
          }}
        />
      )}
