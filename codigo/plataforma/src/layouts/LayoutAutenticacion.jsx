...
export function LayoutAutenticacion({ children, centrado = false, onVolver }) {
  return (
    <main className="relative flex min-h-dvh flex-col items-center justify-start md:justify-center overflow-x-clip p-3 sm:p-6 text-slate-800 font-sans bg-gradient-to-tr from-slate-100 via-sky-50/40 to-blue-50/30 select-none">
      ...
      <div className={`relative z-10 w-full overflow-hidden rounded-[22px] sm:rounded-[32px] border border-slate-200/90 bg-white shadow-[0_24px_60px_-15px_rgba(15,23,42,0.08),0_0_0_1px_rgba(255,255,255,0.9)] backdrop-blur-2xl ${
        centrado 
          ? 'max-w-md p-8' 
        ...
        {!centrado && (
          <section className="relative flex flex-col overflow-hidden bg-[#081b30] px-5 py-5 text-white md:col-span-5 md:px-9 md:py-10">
            ...
            <div className="relative hidden md:block md:mt-10">
            ...
            <div className="relative mt-6 hidden md:flex flex-wrap gap-x-4 gap-y-2 text-[11px] text-slate-300 md:mt-auto">
              <span>Tu inscripción</span><span>Tu grupo</span><span>Tu credencial</span>
            </div>
            <div className="relative mt-5 hidden md:flex items-center gap-2 border-t border-white/10 pt-4 text-[11px] text-slate-400">
        ...
        <section className={`${centrado ? 'w-full' : 'md:col-span-7'} min-w-0 bg-white px-5 py-6 sm:p-8 md:p-11 flex flex-col justify-center`}>
          {children}
        </section>
      ...
      {/* Footer Minimalista */}
      <footer className="mt-4 md:mt-8 px-3 text-center text-[10px] font-bold uppercase tracking-widest text-slate-400">
