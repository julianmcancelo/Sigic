 ...
 */
router.put('/:clave', requiereRol(...ROLES_GESTION), async (req, res) => {
...
router.get('/anfiteatro/estructura/:ceremoniaId', async (req, res) => {
  ...
  try {
    ...
    console.error('ERROR EN ANFITEATRO:', error);
    res.status(500).json({ error: 'Error al obtener la estructura del anfiteatro', detalle: error.message });
  }
});
 ...
 */
router.post('/anfiteatro/estructura/:ceremoniaId', requiereRol(...ROLES_GESTION), async (req, res) => {
  const { ceremoniaId } = req.params;
