...
/**
 * Devuelve el egresado dueño de un invitado (para validar permisos).
 */
async function duenoDeInvitado(req) {
  const result = await db.query('SELECT egresado_id FROM invitados WHERE id = $1', [req.params.id]);
 ...
 */
router.get('/buscar/:codigo', requiereRol(...ROLES_OPERACION), async (req, res) => {
 ...
 */
router.get('/egresado/:egresadoId', requierePersonalOEgresado((req) => req.params.egresadoId, ROLES_LECTURA), async (req, res) => {
  try {
...
router.post('/', async (req, res) => {
  ...
  // Autorización: el token de invitación es credencial suficiente. Si solo
  // viene el ID del egresado, debe acompañarlo una sesión válida (personal
  // de gestión o el propio egresado).
  if (!token && !(esPersonal(req, ROLES_GESTION) || esEgresado(req, egresadoId))) {
    return res.status(401).json({ error: 'Sesión requerida para registrar invitados.' });
  }
 ...
 */
router.put('/:id/presente', requiereRol(...ROLES_OPERACION), async (req, res) => {
  try {
 ...
 */
router.put('/presente-masivo', requiereRol(...ROLES_OPERACION), async (req, res) => {
  const { ids } = req.body;
 ...
 */
router.delete('/:id', requierePersonalOEgresado(duenoDeInvitado), async (req, res) => {
  try {
 ...
 */
router.put('/:id', requierePersonalOEgresado(duenoDeInvitado), async (req, res) => {
